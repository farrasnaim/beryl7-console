    ...
    sheet: function (body, api) {
        ...
        function drawRouter() {
            ...
            if (c && c.packages && c.packages.length) {
                ...
                var upg = ui.act('Update ' + fmt.plural(c.packages.length, 'package'), 'primary sm', function (b) { confirm({ title: 'Update ' + fmt.plural(c.packages.length, 'package') + '?', body: 'apk upgrades exactly these. No reboot.', okText: 'Update' }).then(function (ok) { if (ok) G.act(b, SET, { action: 'pkgupgrade', names: c.packages.map(function (p) { return p.name; }).join(' ') }, { ok: 'Updated.', refresh: [] }).then(function (j) { if (j && j.ok) G.act(null, SET, { action: 'updcheck' }, { ok: false, refresh: ['set'] }); }); }); }); upg.style.marginBottom = '14px'; pane.appendChild(upg);
            }
            ...
            pane.appendChild(rb);
            var lu = el('a', 'act act--quiet act--sm'); lu.href = '/cgi-bin/luci/'; lu.textContent = 'Open LuCI'; lu.style.marginLeft = '8px'; pane.appendChild(lu);
        }
        function drawLog() {
