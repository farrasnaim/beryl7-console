...
- **One bad file in `/etc/nftables.d/` means NO firewall at all, and `/etc/init.d/firewall start` will not fix it.** fw4 loads its whole ruleset in a single nft transaction, so a malformed snippet from any package takes the entire thing down — measured on a real boot: no zone policies, no WAN input filtering, and this console has no login, so the firewall is the only thing keeping it off the internet. Worse, recovery is not where you would reach for it. After a failed boot-time load procd already considers the service started, so `start` answers `The fw4 firewall appears to be already loaded.` and exits 0 while the ruleset stays empty. **`/etc/init.d/firewall restart` is what actually reloads it.** The Overview raises a device-wide alarm for this condition and prints the working command.
- **A loaded `table inet fw4` does not mean a loaded firewall.** After fw4 fails, the table name can still be present — created as an empty shell by another package's post-include — holding zero rules and no `input` chain. Anything checking for the firewall must test for a real chain with a policy, not for the table's existence; the first version of this console's check did the latter and reported "ok" on a router that was wide open.
