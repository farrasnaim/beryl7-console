...
| **Settings** | `/settings/` | Radio configuration (band, channel, width, PHY mode, transmit power, country) with the valid channel/width combinations derived live from what the hardware reports — you cannot select a combination the radio can't do. Plus SSID settings, hostname, timezone, LAN lease settings, device blocking, and radio restart. Under **System**: the running firmware and an on-demand update check via [`owut`](https://openwrt.org/docs/guide-user/installation/attended.sysupgrade), and a **system log** filtered by severity — the honest answer to "why is there no internet" when the only device you have is a phone. |
